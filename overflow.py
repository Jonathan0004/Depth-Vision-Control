...
def _wait_for_pwm_attributes(ch_path: Path, timeout_s: float = 2.0, poll_interval_s: float = 0.01) -> None:
    """Block until the standard PWM sysfs files exist so writes will succeed."""
    required = ("enable", "period", "duty_cycle")
    deadline = time.monotonic() + timeout_s
    while time.monotonic() < deadline:
        if all((ch_path / name).exists() for name in required):
            return
        time.sleep(poll_interval_s)
    missing = [str(ch_path / name) for name in required if not (ch_path / name).exists()]
    raise TimeoutError(f"PWM sysfs entries missing after export: {', '.join(missing)}")


def _ensure_pwm_channel(chip: str, channel: int) -> Path:
    ch_path = _pwm_channel_path(chip, channel)
    if ch_path.is_dir():
        _wait_for_pwm_attributes(ch_path)
        return ch_path
    chip_path = Path(chip)
