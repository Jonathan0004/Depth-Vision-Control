...
pwm_driver = None
last_duty_ns = None  # for optional deadband
...
class PWMDriver:
    """
    Minimal sysfs PWM helper that:
      - Exports once
      - Disables before setting period/duty (required by sysfs)
      - Enables once
      - Keeps 'duty_cycle' file handle open for very fast future writes
    """
    def __init__(self, chip_dir: str, ch_index: int):
        self.chip = chip_dir
        self.idx = ch_index
        self.pwm_dir = f"{self.chip}/pwm{self.idx}"
        self.enable_path = f"{self.pwm_dir}/enable"
