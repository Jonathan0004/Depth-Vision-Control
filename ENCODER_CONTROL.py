...
# PWM configuration — single PWM output for motor speed (pin 32)
motor_pwm_output = {"chip": "/sys/class/pwm/pwmchip3", "channel": 0}  # pin 32
...
# Motor direction control pins (Jetson physical pins 26 and 24)
...
def _set_motor_direction_outputs(direction):
...
def _apply_motor_output(direction, duty_pct, period_ns):
    ...
    # Only drive direction pins high when duty > 0; otherwise ensure both are low.
    effective_direction = direction if duty_ns > 0 else 0

    _set_motor_direction_outputs(effective_direction)
...
def initialise_motor_direction_gpio():
    ...
    try:
        ...
        if mode != GPIO.BOARD:
            GPIO.setmode(GPIO.BOARD)
        GPIO.setup(motor_direction_gpio_pins["forward"], GPIO.OUT, initial=GPIO.LOW)
        GPIO.setup(motor_direction_gpio_pins["reverse"], GPIO.OUT, initial=GPIO.LOW)
...
def shutdown_motor_direction_gpio():
    ...
    if not motor_direction_gpio_initialized:
        return
    try:
        GPIO.output(motor_direction_gpio_pins["forward"], GPIO.LOW)
        GPIO.output(motor_direction_gpio_pins["reverse"], GPIO.LOW)
        GPIO.cleanup([motor_direction_gpio_pins["forward"], motor_direction_gpio_pins["reverse"]])
    except (RuntimeError, ValueError):
        pass
    finally:
        motor_direction_gpio_initialized = False
